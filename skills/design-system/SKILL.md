...
4. **Preview page** — one page/route/screen that renders **every** component in its states: default, hover, focus, disabled, active; empty/loading/error where relevant; responsive. This is the verification gate, in two halves:
   - **Agent half — evidence first.** Build/serve the target, load the preview, and compare what rendered against the extracted component inventory: every component appears, no error output. An agent-browser screenshot is the evidence floor for any stack. A served-HTML / rendered-file fetch substitutes only for **server-rendered output** (a status code alone can't show components). **Exception — client-rendered stacks (React Native, React SPA):** the served HTML is an empty root, so a screenshot or a rendered DOM/tree snapshot is required — a plain HTML fetch is not evidence there. Quote the evidence (URL or file, status, screenshot path, components counted).
   ...
   - **Snapshot the components.** Where the stack has a component-test or snapshot harness, add a minimal render/snapshot test per base component so the preview's correctness is re-checkable later, not eyeball-only.
...
- [ ] Preview demonstrably renders — build/serve ran, the preview loaded (client-rendered stacks proven by screenshot/DOM snapshot, not a bare HTML fetch), and every extracted component appears in its states (default/hover/focus/disabled/active; empty/loading/error; responsive); where a component-test/snapshot harness exists, a minimal render/snapshot test was added per base component
