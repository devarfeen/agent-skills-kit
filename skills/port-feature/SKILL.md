...
- **Narrow retrieval only.** Trace the reference and survey the target with targeted, evidence-backed search (`rg`/`git grep` for the exact route/path/symbol/state), `/feature-discovery`-style. **Never** bulk-read a repo or its `specs/` tree to "find everything". When a graphify knowledge base exists for a repo (`graphify-out/graph.json` at its root, else at the workspace root), query it first to scope the trace and verify graph answers against current code; graph older than ~7 days → recommend the user run `graphify update .`; missing in both places → proceed without it.
...
- **Don't fabricate an issue before coding.** This skill produces a discovery/plan artifact, not a tracker issue. Issues come later, from `/to-tickets` after `/grill-with-docs`.
