...
- A `trigger` query passes when the majority picks `/writing-kit-skills`.
- A `no-trigger` query passes when the majority picks anything else. Its `route` field is
  diagnostic, not pass/fail.

Pass bar: **14/14**. Anything less is a description defect, not a query defect.

Note this skill carries `disable-model-invocation: true` — the eval still runs, because the
description must route correctly for a user reading the catalog and must not steal siblings'
queries in runtimes that surface it.
...
with a written justification. It is not a way to dodge a hard category. This skill gates edits on
`tools/validate.sh` and the trigger-eval sweep, so it has a testable surface — `N/A` does not apply.

Shared anchors and the deep-dive sheets live in `evals/` at the repo root; this file is the local,
self-sufficient copy of what applies to `/writing-kit-skills`.
