...
> **`N/A` on TDD / testing compat:** Read-only, chat-only discovery: it produces no code and gates on no one else's tests, so there is no testable surface. It cites tests only as evidence sources, not as a pass/fail gate.
