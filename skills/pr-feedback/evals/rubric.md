...
- A `trigger` query passes when the majority picks `/pr-feedback`.
...
Pass bar: **18/18**. Anything less is a description defect, not a query defect.

The boundary this set defends: `/code-review` produces a review, this skill consumes one;
`/commit-push-pr` opens or updates a PR, this skill never creates one.
...
with a written justification. This skill applies code fixes and gates on their tests, so category 9
is always scored, never `N/A`.

Shared anchors and the deep-dive sheets live in `evals/` at the repo root; this file is the local,
self-sufficient copy of what applies to `/pr-feedback`.
