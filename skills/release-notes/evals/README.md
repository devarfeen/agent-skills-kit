...
This skill owns its evals. Everything needed to judge `/release-notes` is in this folder.
...
```bash
eval_dir=$(mktemp -d)
bash tools/trigger-evals/build-catalog.sh > "$eval_dir/catalog.md"
python3 tools/trigger-evals/build-queryset.py "$eval_dir"
# run 3 independent judge agents on judge-prompt.md + catalog + queryset
python3 tools/trigger-evals/score.py "$eval_dir/query-manifest.json" j1.jsonl j2.jsonl j3.jsonl
```
...
Before committing that edit, complete the three-judge run and use the scorer's
`--write-snapshot` option to record its provenance. A note saying the old result
is stale does not satisfy the validator's description gate.
