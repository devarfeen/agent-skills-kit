...
> **`N/A` on TDD/testing compat:** The skill produces a markdown handoff prompt with no executable surface; its own verification is re-reading the written file against the approved draft, not test-first work — feature test evidence is owned downstream by grill-with-docs.
