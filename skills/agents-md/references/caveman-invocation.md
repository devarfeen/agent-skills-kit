...
Tool-calling overview: [`tool-calling.md`](tool-calling.md).
...
| **Cursor CLI / IDE** | `/caveman` |
| **Opencode CLI** | `load_skill("caveman")` |

**Trigger Condition:**

- If reading `AGENTS.md` or `CLAUDE.md`/`GEMINI.md` shims, immediately invoke the `caveman` skill.
- In Claude Code or Cursor CLI / IDE, prefer direct `/caveman` invocation. If slash invocation cannot be self-issued from instructions, read the local `caveman` `SKILL.md` and apply its chat-output rules before continuing.
