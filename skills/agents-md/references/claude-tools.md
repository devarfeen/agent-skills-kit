...
- Skills run through the built-in `Skill` tool — there is no separate per-skill tool entry. Invoke directly with `/skill-name`, or let Claude Code auto-load a skill when the request matches its `description`.
