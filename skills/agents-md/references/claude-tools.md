...
- Claude CLI reads `CLAUDE.md` as its canonical workspace file, not `AGENTS.md`. To share rules across runtimes, keep `AGENTS.md` canonical and write a `CLAUDE.md` shim that imports it (e.g. `@AGENTS.md`) — this is the pattern the `agents-md` skill emits.
