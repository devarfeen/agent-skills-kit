...
- Claude CLI reads `CLAUDE.md` as its canonical workspace file, not `AGENTS.md`. To share rules across runtimes, keep `AGENTS.md` canonical and write a `CLAUDE.md` shim that imports it (e.g. `@AGENTS.md`, plus `@CONTEXT.md` / `@MEMORY.md` when present) — this is the pattern the `agents-md` skill emits. Claude auto memory also uses `~/.claude/projects/<project>/memory/MEMORY.md`; that is separate from workspace `MEMORY.md` at `<artifacts-root>`.
