...
- Claude CLI reads `CLAUDE.md` as its canonical workspace file, not `AGENTS.md`. To share rules across runtimes, keep `AGENTS.md` canonical and write a `CLAUDE.md` shim that imports it (e.g. `@AGENTS.md`, `@CONTEXT.md` at `<artifacts-root>`, `@MEMORY.md` at active `<repo-root>` with relative paths in multi-repo) — this is the pattern the `agents-md` skill emits.
- **Memory:** Auto memory is on by default (`autoMemoryEnabled` in settings; `/memory` to toggle). Private store: `~/.claude/projects/<git-root-hash>/memory/MEMORY.md` (first 200 lines / 25KB at startup). **Team SSOT:** git-committed `<repo-root>/MEMORY.md` (≤ ~300 lines). Run `/memory-steward` at session start (light) and on user remember/sync requests (full). Merge private → repo MEMORY on full pass; optional symlink private `MEMORY.md` → repo file (user settings only for `autoMemoryDirectory`). See [`memory-global-defaults.md`](memory-global-defaults.md).
