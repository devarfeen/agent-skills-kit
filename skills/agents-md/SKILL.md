...
The skill version is `v14`. Both generated files carry the marker `<!-- agents-md marker · v14 · re-run /agents-md to regenerate -->` as their first line (the first line of each template asset in `assets/`). Bump it here and in both template assets together whenever these rules change. Pre-`v6` files open with the older `Generated by the agents-md skill` comment — still a valid marker, not hand-authored.
