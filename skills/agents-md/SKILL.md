...
- Emit the `### Matt Skill Routing` subsection only when Matt Pocock's skills are installed (any of `/ask-matt`, `/grill-with-docs`, `/to-prd` resolves in the runtime's skill list); otherwise delete that whole subsection — dead routing rules cost every session tokens.
