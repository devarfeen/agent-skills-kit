...
- **MCP Config:** Keep `.cursor/mcp.json` for Cursor project-level MCP config and `~/.cursor/mcp.json` for Cursor global MCP config. Keep `.mcp.json` for runtimes that read legacy project MCP config. Keep `.agents/mcp_config.json` for Antigravity CLI; remote HTTP MCP entries must use `serverUrl` there.
