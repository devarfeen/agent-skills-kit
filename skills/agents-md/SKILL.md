...
Two rules bind on the verbatim parts:
...
The skill version is `v7`. Both generated files carry the marker `<!-- agents-md marker · v7 · re-run /agents-md to regenerate -->` as their first line (the first line of each template asset in `assets/`). Bump the version here and in both template assets whenever these rules change. Files generated before `v6` open with the older `<!-- Generated by the agents-md skill · v<N> · … -->` line — treat either form as a valid version marker when deciding regeneration, never as hand-authored.
...
- **Migrate `docs/` → `specs/` (ask first).** The kit's artifacts tree is `specs/`; workspaces generated before v7 used `docs/`. When the filled placeholder paths or the workspace root still show a `docs/` artifacts tree (`adr/`, `prompts/`, `qa/`, `port/`, `pixel-audit/`, `integration/`, `design-system/`, `release-notes/`), show what would move and ask the user to confirm the rename. On approval, move the tree (`git mv docs specs` when tracked, plain `mv` otherwise), update the filled placeholder paths to match, and report every moved path. Declined or no response → keep the `docs/` paths as filled; never rename unattended. Move only the artifacts subfolders — leave non-artifact `docs/` content (e.g. a GitHub Pages site) where it is.
