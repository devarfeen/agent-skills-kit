...
- **Coding Tool Aliases:** When asked to run a coding CLI from zsh, use its local high-permission alias: `ccd` for Claude CLI, `cxd` for Codex CLI, `cop` for GitHub Copilot CLI, `ocd` for Opencode CLI, `agd` for Antigravity CLI, and `crd` for `agent`. Do not use tmux-mode aliases for coding tools.
