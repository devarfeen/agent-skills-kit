...
- [ ] Migration (when performed): every stray artifact left `docs/`, merged into the existing `specs/<sub>/` rather than tree-moved over it, no moved artifact still links a `docs/` path, and non-artifact `docs/` content (e.g. `docs/agents/`) is untouched
