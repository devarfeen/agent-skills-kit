...
- A `trigger` query passes when the majority picks `/commit-push-pr`.
...
Pass bar: **20/20**. Anything less is a description defect, not a query defect.
...
self-sufficient copy of what applies to `/commit-push-pr`.
