...
| **plan** | Challenge the plan, capture the decision | `/grill-with-docs` (→ ADR), `/to-spec` (→ spec/PRD), `/wayfinder` (work too big for one session) |
...
5. **`/tdd`** — ask the agent to work the spec's tickets test-first (Red → Green; refactoring rides the review stage).
...
3. **`/to-spec` → `/to-tickets` → `/tdd`** — from here it rejoins Workflow B: spec, thin ticket slices, test-first build. Each UI slice consumes the target's UI library; a missing component goes through `/design-system` (extend), never a page-local one-off.
...
| **New feature** | `/feature-prompt` → `/grill-with-docs` → `/to-spec` → `/to-tickets` → `/tdd` → `/code-review` → `/pixel-audit` → manual QA → `/polish-batch` → `/integration-contract` (multi-project) → `/commit-push-*` |
| **Port a feature** | `/port-feature` → `/grill-with-docs` → `/to-spec` → `/to-tickets` → `/tdd` → `/pixel-audit` → `/commit-push-*` |
