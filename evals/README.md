...
| **Quality rubric** | Is the skill worth loading once triggered? | [`skill-quality-rubric.md`](skill-quality-rubric.md) → scored into `skills/<name>/evals/quality.md` | Any reviewer or agent |
...
`skills/<name>/evals/quality.md`, so a scorecard travels with the thing it
...
- **The frontmatter `description` is the router, and it is under test.**
  The current baseline is 280/280 across three judges. Editing a description
  invalidates that baseline. Improve skill *bodies* and `references/` freely;
  touch a description only when a trigger eval actually fails, and then say in
  the commit body that the baseline is stale.
...
2. Copy [`skill-audit-template.md`](skill-audit-template.md) into
   `skills/<name>/evals/quality.md` and fill it in. One auditor per skill; a
